...
def calcul_des_gains(cartes_joueur,cartes_flop,nouvelle_mise,pari,i,old_mise):
    ...
    if (succes(cartes_joueur,cartes_flop,pari)):
        ...
        a = float(sp_file)
        b = int(a)
       #print("type de b: ",type(b),"valeur de :",b, "type de li :",type(li),"valeur de li :",li)
        
        gain = nouvelle_mise * (1/(b*li))# ici je dois rajouter le i em tour... probleme de type
        old_mise += gain                    
...
def flop(cartes):
    ...
    print(MSG_PIOCHE2,cartes_flop_string[0], cartes_flop_string[1],cartes_flop_string[2])
...
def mise_2(cartes,cartes_joueur,mise):
    ...
    if (reponse == 'O'):
        pari = input(MSG_PARI_MISE)
        print(MSG_SOLDE,int(mise),MSG_BITCOIN,MSG_RESTE_MISE,end="")
        nouvelle_mise = float(input())
...
def poker_un_joueur(s):
    ...
    while(nouvelle_mise >= 0 and reponse != 'N' ):
        ...
        #print(type(nouvelle_mise))
        reponse = input(MSG_CONTINUER)
        reponse = reponse.upper()
